...
Changes: Create and evaluate a torchscript model, compare output to the 
normal model
"""
...
def compare(model, ts_model, device, test_loader):
    model.eval()
    ts_model.eval()
    diff = 0 
    with torch.no_grad():
        for data, target in test_loader:
            X, Ra = data['X'].to(device), data['Ra'].to(device)
            Ri, Ro = data['Ri'].to(device), data['Ro'].to(device)
            normal_output = model(X, Ra.float(), Ri.float(), Ro.float()).squeeze(2).detach().numpy()
            ts_output = ts_model(X, Ra.float(), Ri.float(), Ro.float()).squeeze(2).detach().numpy()
            diff += np.sum(np.abs(normal_output - ts_output))
    return(diff)
...
def main():
    ...
    graph_indir = "C:\\Users\\abdel\\GitHub\\interaction_network\\trackml_data\\processed"#{}_{}\\".format(args.construction, args.pt)
    ...
    disc = validate(ts_model, device, val_loader)
    print("Torchscript model:")
    test(ts_model, device, test_loader, disc=disc)
        
    ts_diff = compare(model, ts_model, device, test_loader)
    print("Net difference between normal model output and torchscript model output:", ts_diff)
